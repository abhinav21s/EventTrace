...
pub fn display_count() ->AnalyticsResponse{
    let analytics=AnalyticsResponse {
        session_count: Session_count.load(Ordering::SeqCst),

        key_count: Counter_key.load(Ordering::SeqCst),

        left_clicks: Counter_mouse_button_left.load(Ordering::SeqCst),

        right_clicks: Counter_mouse_button_right.load(Ordering::SeqCst),

        middle_clicks: Counter_mouse_button_middle.load(Ordering::SeqCst),

        mouse_move: Counter_mouse_move.load(Ordering::SeqCst),

        wheel: Counter_wheel.load(Ordering::SeqCst),
    };
    println!("{:?}",analytics);
    return analytics;
}
